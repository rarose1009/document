...
//리스트
	...
	DlgObject	DlgRes[] = 
	{
		...
		//타이틀
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "특정우선점검 개소 선택"),
...
//--------------------------------------------------------------------------------------------------------
	void OnList(quad lParam)
	{
		...
		if( i > -1 )
		{
			g_Sql_SaveSeq( "SELECT ROWID FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ", SREACH_FILE);
			g_nActIndex = lParam;
...
Finally:
	...
	char szRow[300];
		...
		{
			Str_Cpy(szRow, "SELECT ROWID FROM SPECIAL_DATA WHERE ( ( CHK_RSLT != '20' AND CHK_RSLT != '21' AND CHK_RSLT != '40' ) or CHK_RSLT is null ) AND LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ");
			g_nWorkFlag = 306;
		}
		...
		if( Str_Cmp(m_szCobSel, "미검") == 0 )
		{
			Str_Cpy( m_szSql[0], "SELECT ifnull(TOWN,' ') ||' '|| ifnull(ADDR1_M,' ') ||'-'|| ifnull(ADDR1_S,' ') ||'\n'|| ifnull(SECOND_ADDR,' '), PRESS_FLAG, CHK_RSLT,  ifnull(TOWN,' ') ||' '|| ifnull(ADDR1_M,' ') ||'-'|| ifnull(ADDR1_S,' ') ||' '|| ifnull(SECOND_ADDR,' ') FROM SPECIAL_DATA WHERE ( ( CHK_RSLT != '20' AND CHK_RSLT != '21' AND CHK_RSLT != '40' ) or CHK_RSLT is null ) AND LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 LIMIT ? , ?");
			Str_Cpy( m_szSql[1], "SELECT ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||'\n'|| ifnull(SECOND_ADDR,' '), PRESS_FLAG, CHK_RSLT, ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||' '|| ifnull(SECOND_ADDR,' ') FROM SPECIAL_DATA WHERE ( ( CHK_RSLT != '20' AND CHK_RSLT != '21' AND CHK_RSLT != '40' ) or CHK_RSLT is null ) AND LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 LIMIT ? , ?");
		}
		else
		{
			Str_Cpy( m_szSql[0], "SELECT ifnull(TOWN,' ') ||' '|| ifnull(ADDR1_M,' ') ||'-'|| ifnull(ADDR1_S,' ') ||'\n'|| ifnull(SECOND_ADDR,' '), PRESS_FLAG, CHK_RSLT, ifnull(TOWN,' ') ||' '|| ifnull(ADDR1_M,' ') ||'-'|| ifnull(ADDR1_S,' ') ||' '|| ifnull(SECOND_ADDR,' ') FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 LIMIT ?, ?");
			Str_Cpy( m_szSql[1], "SELECT ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||'\n'|| ifnull(SECOND_ADDR,' '), PRESS_FLAG, CHK_RSLT, ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||'\n'|| ifnull(SECOND_ADDR,' ') FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 LIMIT ?, ?");
		}
	}
	...
	void Cmb_Result(void)
	{
		...
		else if( Str_Cmp(m_szCobSel, "미검") == 0 )
		{
			...
			g_Sql_RetStr( "SELECT COUNT(*) FROM SPECIAL_DATA WHERE CHK_RSLT = '20' AND (( CHK_RSLT <> '20' AND CHK_RSLT <> '21' ) OR CHK_RSLT IS NULL) AND LENGTH(PREUSE_CHK_WHY) > 0", 4 , sztmp );
			EditCtrl_SetStr(Get_hDlgCtrlByID(TXT_DATA3), sztmp); 
			
			Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
			g_Sql_RetStr( "SELECT COUNT(*) FROM SPECIAL_DATA WHERE CHK_RSLT = '21' AND (( CHK_RSLT <> '20' AND CHK_RSLT <> '21' ) OR CHK_RSLT IS NULL) AND LENGTH(PREUSE_CHK_WHY) > 0 ", 4 , sztmp );
			EditCtrl_SetStr(Get_hDlgCtrlByID(TXT_DATA4), sztmp); 
...
//---------------------------------------------------------------------------------------------------------------
	void QuickList(long nBtnID)
	{
		...
		{
			JSON_SetValue ( js, 'C', "title", "특정우선점검 개소선택");
			JSON_SetValue ( js, 'C', "dbname", "APP_DBF/SCGGAS.db");
			JSON_SetValue ( js, 'C', "query", "SELECT ifnull(TOWN,' ') ||' '|| ifnull(ADDR1_M,' ') ||'-'|| ifnull(ADDR1_S,' ') ||' '|| ifnull(SECOND_ADDR,' '), ifnull(SPECIAL_NM, '') ||' / 압력:'|| Case When PRESS_FLAG = 'L' Then '저압' When PRESS_FLAG = 'M' Then '중압' When PRESS_FLAG = 'R' Then '중간압' Else '' End ||' / 결과:'|| Case When CHK_RSLT = '20' Then '적합' When CHK_RSLT = '21' Then '부적합' When CHK_RSLT = '40' Then '제외' Else '미검' End FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ");
		}
		else
		{
			JSON_SetValue ( js, 'C', "title", "특정우선점검 개소선택");
			JSON_SetValue ( js, 'C', "dbname", "APP_DBF/SCGGAS.db");
			JSON_SetValue ( js, 'C', "query", "SELECT ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||' '|| ifnull(SECOND_ADDR,' '), ifnull(SPECIAL_NM, '') ||' / 압력:'|| Case When PRESS_FLAG = 'L' Then '저압' When PRESS_FLAG = 'M' Then '중압' When PRESS_FLAG = 'R' Then '중간압' Else '' End ||' / 결과:'|| Case When CHK_RSLT = '20' Then '적합' When CHK_RSLT = '21' Then '부적합' When CHK_RSLT = '40' Then '제외' Else '미검' End FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ");
		}
...
Finally:
	...
	void Grid_QuickList(long nBtnID)
	{
		...
		if( nBtnID == BID_QUICK_ADDR )
		{
			Str_Cpy(m_szQuickSql, "SELECT IFNULL(TOWN,' ') ||' '|| IFNULL(ADDR1_M,' ') ||'-'|| IFNULL(ADDR1_S,' ') ||' '|| IFNULL(SECOND_ADDR,' ') \
		...
		{
			Str_Cpy(m_szQuickSql, "SELECT ifnull(NEW_ROAD_NM,' ') ||' '|| ifnull(NEW_ADDR_M,' ') ||'-'|| ifnull(NEW_ADDR_S,' ') ||' '|| ifnull(SECOND_ADDR,' ') \
									    , CASE WHEN PRESS_FLAG = 'L' THEN '저압' WHEN PRESS_FLAG = 'M' THEN '중압' WHEN PRESS_FLAG = 'R' THEN '중간압' ELSE '' END \
									    , CASE WHEN CHK_RSLT = '20' THEN '적합' WHEN CHK_RSLT = '21' THEN '부적합' WHEN CHK_RSLT = '40' THEN '제외' ELSE '미검' END \
									 FROM SPECIAL_DATA ");
		}
		...
		if( Str_Cmp(m_szCobSel, "미검" ) == 0 )
		{
			Str_Cat(m_szQuickSql, " WHERE ( CHK_RSLT != '20' AND CHK_RSLT != '21' AND CHK_RSLT != '40' ) or CHK_RSLT is null AND LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ");
		}
		else //if( Str_Cmp(m_szCobSel, "전체" ) == 0 )
		{
		...
		if (h)
		{
			...
			JSON_SetValue	(h		, 'C', "title"			, "특정우선점검 개소 선택");
	...
	void Get_Index(void)
	{
		...
		SPRINT(szSql, "SELECT PARAM9 FROM RCV_LOG  WHERE GUBUN = '5' ", 0, 0, 0);		
	...
	void SetComboIdx(void)
	{
		...
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		SPRINT(szSql, "SELECT ROWID FROM SPECIAL_DATA WHERE LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ", 0, 0, 0);
		g_Sql_SaveSeq( szSql, SREACH_FILE);
		
		//g_szCHK_EXEC_NUM
		
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		SPRINT(szSql, "SELECT ROWID FROM SPECIAL_DATA WHERE CHK_EXEC_NUM = '%s' AND LENGTH(PREUSE_CHK_WHY) > 0 ORDER BY TOWN, ADDR1_M, ADDR1_S, SECOND_ADDR, ZIP_NO1, ZIP_NO2 ", g_szCHK_EXEC_NUM, 0, 0);
