...
#include "DefEvent.h"
	...
	DEF_BUTTON_ID(BID_REQ)	   // 청구방법(이동고객센터 이동)
	DEF_BUTTON_ID(BID_REMOTE)  // 원격지침

	DEF_BUTTON_ID(BID_MTR_ID_NUM) // 기물
	DEF_BUTTON_ID(BID_NOTICE)	  // 공지
...
#define REFRESH_MSGY2 DMS_Y(STARTY + 500)
	...
	long Rcv_remote_select(void);
	long TR25011(void);
	long TR25013(void);
	long SelfSelect(void);
	long RemoteSelect(void);
	...
	DlgObject DlgRes[] =
		{
			...
			DLG_BUTTON(STARTX + 140, STARTY + 5, 160, 50, 0, 0, BUTSTY_BOLD, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC, "", BID_NOTICE, "공지"),
			DLG_BUTTON(STARTX + 660, STARTY + 5, 160, 50, 0, 0, BUTSTY_BOLD, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC, "", BID_PHOTO, "사진"),
			...
			DLG_BUTTON(STARTX + 780, STARTY + 245, 220, 50, 0, 0, BUTSTY_BOLD, BUTSTY_BORDER, RED, TXTINCTRLBK, CALL_FUNC, "", BID_REQ, ""), // 청구방법(이동고객센터)
	...
	void OnInit(char bFirst)
	{
		...
		case INIT_MAIN:
			...
			for (i = 0; GM_CODE[i].Str[0] != 0; i++)
			{
				ListCtrl_AddItem(Get_hDlgCtrlByID(CMB_RESULT + 2), GM_CODE[i].Str, 0, ICON_NONE);
			}
			...
			else
			{
				...
				if (Str_Cmp(stGm.SELF_GMTR_YN, "Y") == 0)
				{
					...
					else if (Str_Cmp(stGm.MTR_LOC_FLAG, "10") == 0 && Str_Cmp(stGm.GMTR_JOB_YMD_END, szToday) == 0 && lEndDate > 0)
					{
				...
				else if (Str_Cmp(stGm.REMOTE_GMTR_YN, "Y") == 0)
				{
					...
					// SPRINT(szMsg, "[%s]", stGm.AMI_REQ_YM, 0, 0);
				...
				// else if (Str_Cmp(stGm.REMOTE_GMTR_YN, "O") == 0)
				// {
				// 	if (Str_Cmp(stGm.AMI_REQ_YM, "03") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "04") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "11") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "12") == 0)
				// 	{
				// 		MessageBoxEx(CONFIRM_OK, "AMI 원격검침 세대이지만 \n 3월,4월,11월 그리고 12월에는 \n PDA검침 진행됩니다.");
				// 		DlgCtrl_SetEnable(this->m_hDlg, Get_iDlgCtrlByID(EDT_VC), TRUE);
				// 		DlgCtrl_SetGray(this->m_hDlg, Get_iDlgCtrlByID(EDT_VC), FALSE);
			...
			{
				DlgCtrl_SetVisible(this->m_hDlg, Get_iDlgCtrlByID(BID_REQ), FALSE);
			...
			{
				DlgCtrl_SetVisible(this->m_hDlg, Get_iDlgCtrlByID(BID_REQ), TRUE);
		...
		if (m_bFirst == INIT_MAIN)
		{
			...
			else
			{
				if (Str_AtoI(EditCtrl_GetStr(Get_hDlgCtrlByID(EDT_VC))) > 0)
				{
					...
					i = SearchStr(EditCtrl_GetStr(Get_hDlgCtrlByID(CMB_RESULT)), GM_CODE);

					if (i >= 0)
					{
						Str_Cpy(szMtr_Code, GM_CODE[i].Code);
					}
			...
			if (Get_iDlgCtrlByID(EDT_VC) == m_lEditCtrlIndex)
			{
				...
				if (Str_Cmp(stGm.MTR_NUM, "302094293") == 0)
				{
					if(nLen > 7)
					{
						...
						Str_Cpy(strMsg, "해당 계량기 지침 자릿수는 최대 7자리입니다.\n ");
						Str_Cat(strMsg, "\n계량기번호 : ");
				...
				else
				{
					...
					if (i == -1)
					{
						Str_Cpy(m_szMtr_Code, GM_CODE[0].Code);
					}
					else
					{
						Str_Cpy(m_szMtr_Code, GM_CODE[i].Code);
					}
	...
	void SetBtnImg(void)
	{
		...
		if (m_bFirst == INIT_MAIN)
		{
			...
			IconCtrl_SetImage(Get_hDlgCtrlByID(ICON_TITLE), 0, g_pImgMainTtl_GM);
	...
	void OnButton(long ID)
	{
		...
		case BID_PHOTO:
			Mem_Set((byte *)m_szPhoto_CompensType, 0x00, sizeof(m_szPhoto_CompensType));
			Str_Cpy(m_szPhoto_CompensType, "0");
			...
			if (nEvent == UART_CAMERA)
			{
				if (nData == 1)
				{
					...
					if (FFS_Exist(szfilenm))
					{
						...
						{
							SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VC_YN = 'S' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
							g_Sql_DirectExecute(szTmp);
						...
						{
							SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VA_YN = 'S' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
							g_Sql_DirectExecute(szTmp);
						...
						{
							SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VM_YN = 'S' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
							g_Sql_DirectExecute(szTmp);
	...
	void OnSelect(long Index)
	{
		...
		if (m_bFirst == INIT_MAIN)
		{
			...
			// 지침 값을 입력 후 코드값을 바?f을 경우 계산이 다시 되게 하기위해서.
			if (m_lUserData == 10)
			{
				Mem_Set((byte *)stGm.INDI_SET_VC, 0x00, sizeof(stGm.INDI_SET_VC));
				Str_Cpy(stGm.INDI_SET_VC, EditCtrl_GetStr(Get_hDlgCtrlByID(EDT_VC)));
	...
	void Set_Index(void)
	{
		...
		Mem_Set((byte *)sztmp, 0x00, sizeof(sztmp));
		SPRINT(sztmp, "UPDATE RCV_LOG SET PARAM8 =%d WHERE GUBUN = '1' ", g_lindex, 0, 0);
		g_Sql_DirectExecute(sztmp);
	}
	...
	void SetStyle(void)
	{
		if (m_bFirst == INIT_MAIN)
		{
			...
			if (10 == m_lUserData)
			{
				if (Str_Cmp(m_szgr, "") == 0)
				{
					EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), GM_CODE[0].Str);
	...
	void SetData(void)
	{
		...
		if (sql->Next(sql) == TRUE)
		{
			...
			sql->GetValue(sql, i++, 'U', (long *)stGm.GMTR_JOB_YMD_END, 8 + 1, DECRYPT);
	...
	Finally:
		...
		Str_Cpy(m_szIndiVc, stGm.INDI_SET_VC);
	...
	void REDRAW(void)
	{
		...
		/*****************************************/
		// if (Str_Cmp(stGm.REMOTE_GMTR_YN, "O") == 0)
		// {
		// 	if (Str_Cmp(stGm.AMI_REQ_YM, "03") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "04") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "11") == 0 || Str_Cmp(stGm.AMI_REQ_YM, "12") == 0)
		// 	{
		...
		*/
		...
		if (Str_Cmp(stGm.EBPP_FLAG, "Y") == 0)
		{
			ButCtrl_SetText(Get_hDlgCtrlByID(BID_REQ), "메일");
		}
		else if (Str_Cmp(stGm.EBPP_FLAG, "L") == 0)
		{
			ButCtrl_SetText(Get_hDlgCtrlByID(BID_REQ), "LMS");
		}
		else if (Str_Cmp(stGm.EBPP_FLAG, "M") == 0)
		{
			ButCtrl_SetText(Get_hDlgCtrlByID(BID_REQ), "모바일");
		}
		else if (Str_Cmp(stGm.EBPP_FLAG, "S") == 0)
		{
		...
		/* 검침결과가 있으면 취소 red -> S, I(I는 순수미검침 대상이 저장?瑛뻑㎱?flag) */
		...
		if (g_lDataflag == 0)
		{
			...
			// 작업코드 콤보박스 세팅
			if (Str_Cmp(stGm.SEND_YN, "E") == 0)
			{
				i = SearchCD(stGm.MTR_WORK_CODE, GM_CODE_ORI);

				if (i == -1)
				{
					EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), GM_CODE_ORI[0].Str);
				}
				else
				{
					EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), GM_CODE_ORI[i].Str);
				}
			}
			else
			{
				i = SearchCD(stGm.MTR_WORK_CODE, GM_CODE);
				...
				{
					EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), GM_CODE[i].Str);
				}
			}
	...
	bool Gm_ChkRule(void)
	{
		...
		/****************************************************/
		if (Str_Cmp(stGm.SEND_YN, "E") == 0 && Str_Cmp(EditCtrl_GetStr(Get_hDlgCtrlByID(EDT_VC)), m_szIndiVc) == 0)
		{
		...
		if (Str_AtoI(m_szIndiQty) > 0)
		{
			...
			if (Str_AtoI(stGm.MTR_RESOU_FLAG) != 10)
			{
				...
				SPRINT(szSql, "SELECT PARAM10 FROM RCV_LOG WHERE GUBUN = '1' ", 0, 0, 0);
				g_Sql_RetStr(szSql, 4, sztmp);
			...
			else
			{
				...
				SPRINT(szSql, "SELECT PARAM9 FROM RCV_LOG WHERE GUBUN = '1' ", 0, 0, 0);
				g_Sql_RetStr(szSql, 4, sztmp);
	...
	long Save_Gm(void)
	{
		...
		if (Str_Cmp(stGm.MTR_WORK_CODE, "L") != 0)
		{
			...
			if (Str_AtoI(stGm.GRD_TITLE) >= 100)
			{
				hstmt = sql->CreateStatement(sql, "UPDATE GUMDATA \
	...
	long Save_factor(void)
	{
		...
		{
			if (EditCtrl_GetStrLen(Get_hDlgCtrlByID(EDT_DATA4)) == 0 || EditCtrl_GetStrLen(Get_hDlgCtrlByID(EDT_DATA6)) == 0)
			{
		...
		{
			if (EditCtrl_GetStrLen(Get_hDlgCtrlByID(EDT_DATA4)) == 0 || EditCtrl_GetStrLen(Get_hDlgCtrlByID(EDT_DATA5)) == 0 || EditCtrl_GetStrLen(Get_hDlgCtrlByID(EDT_DATA6)) == 0)
			{
				MessageBoxEx(CONFIRM_OK, "상시 계수를 입력하세요.");
				goto Finally2;
			}
	...
	void SetTotCnt(void)
	{
		...
		{
			Mem_Set((byte *)szSql, 0x00, sizeof(szSql));
			SPRINT(szSql, "SELECT COUNT(*) FROM GUMDATA WHERE SEND_YN = 'N' OR SEND_YN = 'I' OR SEND_YN = '' OR SEND_YN IS NULL ORDER BY DONG_ORD, HOUSE_NUM_ORD, MTR_COURSE", 0, 0, 0);
		...
		{
			Mem_Set((byte *)szSql, 0x00, sizeof(szSql));
			SPRINT(szSql, "SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE = '2' ORDER BY DONG_ORD, HOUSE_NUM_ORD, MTR_COURSE", 0, 0, 0);
			g_Sql_RetInt(szSql, &m_ltotcnt);
		}
		// 나머지
		else
		{
			Mem_Set((byte *)szSql, 0x00, sizeof(szSql));
			SPRINT(szSql, "SELECT COUNT(*) FROM GUMDATA ORDER BY DONG_ORD, HOUSE_NUM_ORD, MTR_COURSE ", 0, 0, 0);
			g_Sql_RetInt(szSql, &m_ltotcnt);
		}
	}
	...
	long RCV311318(void)
	{
		...
		g_Sql_RetStr("SELECT PARAM6 FROM RCV_LOG WHERE GUBUN = '1'", 4, m_szZonemanagenum);
		//g_Sql_RetStr("SELECT ZONE_MANAGE_NUM FROM GUMDATA LIMIT 1", 4, m_szZonemanagenum);
	...
	long TR25005(void)
	{
		...
		{
			Mem_Set((byte *)szSql, 0x00, sizeof(szSql));
			SPRINT(szSql, "UPDATE GUMDATA SET SMS_GMTR_MAN_YN = 'N' WHERE USE_CONT_NUM = '%s' ", stGm.USE_CONT_NUM, 0, 0);
			g_Sql_DirectExecute(szSql);
				...
				Str_Cmp(stGm.GMTR_SEND_STS, "80") == 0)
			{
				...
				if (Str_Cmp(stGm.INDI_SET_VC, "") == 0 && (Str_Cmp(stGm.MTR_WORK_CODE, "2") != 0 && Str_Cmp(stGm.MTR_WORK_CODE, "C") != 0))
				{
					if (Str_Cmp(stGm.GMTR_SEND_STS, "80") == 0)
					{
						...
						if (Str_Cmp(stGm.MTR_WORK_CODE, "L") == 0 || (Str_Cmp(stGm.SEND_YN, "Y") != 0 && Str_Cmp(stGm.SEND_YN, "S") != 0))
						{
							Mem_Set((byte *)stGm.SEND_YN, 0x00, sizeof(stGm.SEND_YN));
							Str_Cpy(stGm.SEND_YN, "E");

							Mem_Set((byte *)stGm.INDI_SET_VC, 0x00, sizeof(stGm.INDI_SET_VC));
							Str_Cpy(stGm.INDI_SET_VC, stGm.RCV_SMS_DESC);

							Mem_Set((byte *)m_szIndiVc, 0x00, sizeof(m_szIndiVc));
							Str_Cpy(m_szIndiVc, stGm.RCV_SMS_DESC);
						}
					}
				}
			}
			...
			if (Str_Cmp(stGm.GMTR_SEND_STS, "") != 0 && Str_Cmp(stGm.GMTR_SEND_STS, "null") != 0)
			{
				...
				if (i == -1)
				{
						...
						Str_Cmp(stGm.GMTR_SEND_STS, "80") == 0)
					{
						...
						*/
						if ((Str_Cmp(stGm.MTR_WORK_CODE, "2") != 0 && Str_Cmp(stGm.MTR_WORK_CODE, "C") != 0) && Str_Cmp(stGm.SEND_TYPE, "30") == 0)
						{
							if (Str_Cmp(stGm.GMTR_SEND_STS, "80") == 0)
							{
								EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), "사전회신");
	...
	long Rcv_remote_select(void)
	{
		...
		g_pjcomm = g_CreateJsonHead(g_pjcomm, szUrl, 25013, FALSE);
		...
		callBackfunc = TR25013;
		ret = HTTP_DownloadData(szUrl, "FR25013_IN", "FR25013", sndbuf, sztmp);
		return 1;
	}

	/*=======================================================================================
	함수명 : TR25013
	기  능 : FR25013 CALLBACK
	Param  :
	Return :
	========================================================================================*/
	long TR25013(void)
	{
		if (g_Chk_Json(25013) >= 0)
		{
			RemoteSelect();
			ON_DRAW();
			return 1;
	...
	long RemoteSelect(void)
	{
		...
		Mem_Set((byte *)szJsonpath, 0x00, sizeof(szJsonpath));
		SPRINT(szJsonpath, "%sFR%d_Res.JSON", SYSTEM_COMM, 25013, 0);

		if (FFS_Exist(szJsonpath) != TRUE)
		{
			Mem_Set((byte *)szJsonpath, 0x00, sizeof(szJsonpath));
			SPRINT(szJsonpath, "%sFR%d.JSON", SYSTEM_COMM, 25013, 0);
		}
					...
					if (Str_Len(szitem) > 0)
					{
						...
						if (hitem != NULL)
						{
							...
							if (ret > 0)
							{
								...
								EditCtrl_SetStr(Get_hDlgCtrlByID(CMB_RESULT), "원격검침");
	...
	long TR26002(void)
	{
		...
		if (g_Chk_Json(26002) >= 0)
		{
			...
			if (Str_Cmp(m_szPhoto_CompensType, "1") == 0)
			{
				SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VC_YN = 'Y' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
				g_Sql_DirectExecute(szTmp);
			...
			{
				SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VA_YN = 'Y' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
				g_Sql_DirectExecute(szTmp);
			...
			{
				SPRINT(szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VM_YN = 'Y' WHERE ROWID = '%s'", stGm.ROWID, 0, 0);
				g_Sql_DirectExecute(szTmp);
	...
	long Chk_Photo(void)
	{
		...
		SPRINT(szSql, "SELECT COUNT(1) FROM GUMDATA WHERE MTR_NUM  ='%s' AND PHOTO_MTR_YN = 'Y'", stGm.MTR_NUM, 0, 0);
	...
	long TR26001(void)
	{
		...
		if (g_Chk_Json(26001) >= 0)
		{
			...
			MessageBoxEx(CONFIRM_OK, "사진이 저장되었습니다. \n '저장'버튼을 다시 눌러주세요.");
