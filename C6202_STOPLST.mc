...
//검침 리스트
	...
	DlgObject	DlgRes[] = 
	{
		...
		//타이틀
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "중지리스트"),
		...
		DLG_BUTTON(0,          STARTY+831, 333, 68, 0, 0, BUTSTY_BOLD, BUTSTY_ROUNDBORDER , BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_UP, "▲"),
		DLG_BUTTON(STARTX+333, STARTY+831, 333, 68, 0, 0, BUTSTY_BOLD, BUTSTY_ROUNDBORDER , BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_DOWN, "▼"),
		DLG_BUTTON(STARTX+666, STARTY+831, 334, 68, 0, 0, BUTSTY_BOLD, BUTSTY_ROUNDBORDER , BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_RCV, "자료수신"),
	...
	DlgObject	DlgRes_P[] = 
	{
		...
		//타이틀
		DLG_TEXT(P_TX, P_TY, P_STWD, P_STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, WHITE, TRANSPARENT, TXT_TITLE, "중지리스트"),
		...
		DLG_BUTTON(0,          STARTY+825, 250, 70, 0, 0, BUTSTY_BOLD, BUTSTY_ROUNDBORDER , BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_UP, "▲"),
		DLG_BUTTON(STARTX+265, STARTY+825, 250, 70, 0, 0, BUTSTY_BOLD, BUTSTY_ROUNDBORDER , BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_DOWN, "▼"),
		...
		DLG_COMBO_LIST (STARTX, 	   STARTY+260, 425, 290, 125, 69, TXTFRCOLOR, TXTINCTRLBK, CALL_FUNC, "", CMB_DATA1, 30),
	};
...
//PRINT(">>>>>> lParam : %d , i : %d",lParam,i,0);
		if( i > -1 )
		{
			g_Sql_SaveSeq( "SELECT ROWID FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' ORDER BY TOWN, ADDR1_M", SREACH_FILE);

			g_nActIndex = lParam;
			g_lindex = g_SREACHFILE_GetSeq(SREACH_FILE, g_nActIndex);

			Mem_Set( (byte*)g_szUSE_CONT_NUM, 0x00, sizeof(g_szUSE_CONT_NUM));
			SPRINT(szTmp, "SELECT USE_CONT_NUM FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' AND ROWID = '%d'", g_lindex, 0, 0);
			g_Sql_RetStr(szTmp, 20, g_szUSE_CONT_NUM);
...
//------------------------------------------------------------------------------------------
	long Rcv_Data(void)
	{
		...
		g_pjcomm = g_CreateJsonHead(g_pjcomm, szUrl, 83112, FALSE);
		...
		callBackfunc = TR83112; //통신완료 후에 호출 함수
		ret = HTTP_DownloadData(szUrl, "FR83112_IN",  "FR83112" , sndbuf, sztmp );
		return 0 ;
	}
...
//------------------------------------------------------------------------------------------
	long TR83112(void)	
	{
		...
		if( g_Chk_Json(83112) >= 0)
		{
			g_Sql_RetInt("SELECT COUNT(*) FROM C6202_STOP_OBJ", &ntotcnt);
			
			if(ntotcnt > 0 )
			{
				g_Sql_DirectExecute("UPDATE C6202_STOP_OBJ SET VISIT_FLAG = 'N', CUT_FLAG = 'N', AUTO_FLAG = 'N', RELE_FLAG = 'N' \
				                    , RCEI_FLAG = 'N', CUST_FLAG = 'N' ");
...
FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' ORDER BY TOWN, ADDR1_M");
			}
			else
			{
				Str_Cpy(m_szQuickSql, "SELECT \
...
FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' ORDER BY TOWN, ADDR1_M");
		...
		{
			Str_Cpy(m_szQuickSql, "SELECT PAY_FIRM_NM, NONPAY_MM_CNT, ALL_UNPAY, VISIT_FLAG FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' ORDER BY TOWN, ADDR1_M");
		}
		else if( Str_Cmp(EditCtrl_GetStr(Get_hDlgCtrlByID(CMB_DATA1)), "성명") == 0 )
		{
			Str_Cpy(m_szQuickSql, "SELECT PAY_CUST_NM, NONPAY_MM_CNT, ALL_UNPAY, VISIT_FLAG FROM C6202_STOP_OBJ WHERE STOP_FLAG = 'Y' ORDER BY TOWN, ADDR1_M");
		}
		...
		if (h)
		{
			...
			JSON_SetValue	(h		, 'C', "title"			, "중지 리스트");
