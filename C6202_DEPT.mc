...
//	#define USE_ON_SELECT
	...
	DlgObject	DlgRes[] = 
	{
		...
		DLG_TEXT(POPTX, POPTY, POPSTWD, POPSTHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "채무불이행 이력조회"),
		DLG_BUTTON(BTNX_7, P_BTNY, BTNWD_2, BTNHT_2, 0, 0, EDITSTY_BORDER, 0, BTNMENUFRCOLOR, BTNCTRLMNBKCOLOR, CALL_FUNC , "", BID_OK, ""),

		DLG_BUTTON(STARTX+600,  STARTY+750, 400, 90, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_OK, "확 인"),
		...
		DLG_TEXT(STARTX,       STARTY+181, 400, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTTTLFRCOLOR, TXTCTRLBK, TXT_DATA7, "의뢰금액"),
		DLG_TEXT(STARTX+400,   STARTY+181, 600, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTFRCOLOR, TXTINCTRLBK, TXT_DATA8, ""),
		DLG_TEXT(STARTX,       STARTY+251, 400, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTTTLFRCOLOR, TXTCTRLBK, TXT_DATA9, "신용구분"),
	...
	DlgObject	DlgRes_P[] = 
	{
		...
		//타이틀
		DLG_TEXT(P_TX, P_TY, P_STWD, P_STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, WHITE, TRANSPARENT, TXT_TITLE, "채무불이행 이력조회"),
		DLG_BUTTON(P_BTNX, P_BTNY, P_BTNWD, P_BTNHD, 0, 0, BUTSTY_BORDER, 0, BTNMENUFRCOLOR, BTNCTRLMNBKCOLOR, CALL_FUNC , "", BID_OK, ""),

		DLG_BUTTON(STARTX+600,  STARTY+740, 400, 100, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_OK, "확 인"),
		...
		DLG_TEXT(STARTX,       STARTY+170, 400, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTTTLFRCOLOR, TXTCTRLBK, TXT_DATA7, "의뢰금액"),
		DLG_TEXT(STARTX+400,   STARTY+170, 600, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTFRCOLOR, TXTINCTRLBK, TXT_DATA8, ""),
		DLG_TEXT(STARTX,       STARTY+240, 400, 70, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTTTLFRCOLOR, TXTCTRLBK, TXT_DATA9, "신용구분"),
...
//------------------------------------------------------------------
	long Rcv_Dept(void)
	{
		...
		g_pjcomm = g_CreateJsonHead(g_pjcomm, szUrl, 82239, FALSE);
		...
		callBackfunc = TR82239; //통신완료 후에 호출 함수
		ret = HTTP_DownloadData(szUrl, "FR82239_IN",  "FR82239" , sndbuf, sztmp );
		return 0 ;
	}
					...
					if( Str_Len(szitem) > 0 )
					{
						...
						if( hitem != NULL)
						{
							...
							if( ret > 0 )
							{
								...
								//신용구분
								Mem_Set((byte*) szCol, 0x00, sizeof( szCol) );
								szcode = (char*)JSON_GetArrayItem ( hitem, 0, "credit_job_flag", szType);
								SQL_ConvMbStr(szCol, szcode);
								...
								//의뢰금액
								Mem_Set((byte*) szCol, 0x00, sizeof( szCol) );
								nCol = (long*)JSON_GetArrayItem ( hitem, 0, "treat_amt", szType);
								nCnt = *nCol;
								Str_ItoA( nCnt, szCol, 10);
