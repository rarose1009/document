...
//	#define USE_ON_POINTING
	...
	DlgObject	DlgRes[] = 
	{
		...
		//타이틀
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "해제의뢰취소"),

		DLG_BUTTON(STARTX,       STARTY+65, 250, 110, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_ADDR, "지번\n주소"),
		DLG_BUTTON(STARTX+500,  STARTY+472, 250, 60, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_PREVLST, "▲"),
		DLG_BUTTON(STARTX+750,  STARTY+472, 250, 60, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_NEXTLST, "▼"),
		DLG_BUTTON(STARTX+600,  STARTY+829, 400, 70, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_CLEARCANCEL, "해제의뢰취소"),

		DLG_TEXT(STARTX+250,    STARTY+65, 750,110, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTFRCOLOR, TXTINCTRLBK, TXT_DATA1, ""),
		DLG_TEXT(STARTX,       STARTY+472, 300, 60, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TXTFRCOLOR, TXTINCTRLBK, TXT_PAGE, m_szCnt_page),
	...
	DlgObject	DlgRes_P[] = 
	{
		...
		//타이틀
		DLG_TEXT(P_TX, P_TY, P_STWD, P_STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, WHITE, TRANSPARENT, TXT_TITLE, "해제의뢰 취소"),
		...
		DLG_BUTTON(STARTX+635,  STARTY+800, 350, 60, 0, 0, 0, BUTSTY_BORDER, BTNMENUFRCOLOR, BTNCTRLBKCOLOR, CALL_FUNC , "", BID_CLEARCANCEL, "해제의뢰취소"),
...
//------------------------------------------------------------------
	long Snd_ClearCancel(void)
	{
		...
		{
			MessageBoxEx(CONFIRM_OK, "해제의뢰 취소사유를 입력하세요.");
			return -1;
		}
		...
		if( MessageBoxEx (CONFIRM_YESNO, "해제의뢰 취소를 등록 하시겠습니까?")  != MB_OK)
		{
			return -1;
		}
		
		Mem_Set((byte*)&m_stClearCancel, 0x00, sizeof(m_stClearCancel));
		
		SQL_ConvUcStr( m_stClearCancel.ACTIVE_RSLT_DESC, szActive_rslt_desc );
		Str_Cpy(m_stClearCancel.NONPAY_ACTIVE_FLAG, "330");
		Str_Cpy(m_stClearCancel.NONPAY_CENTER_CD, stUserinfo.szcenter_cd);
		SQL_ConvUcStr( m_stClearCancel.TAKER_NM, stUserinfo.sznm_kor );
		Str_Cpy(m_stClearCancel.CUT_JOB_FLAG, "20");
		Str_Cpy(m_stClearCancel.MTR_NUM, DlgTbl_GetStr( ID_TBLLIST, m_lActIndex, 6));
		...
		g_pjcomm = g_CreateJsonHead(g_pjcomm, szUrl, 81208, FALSE);

		JSON_SetValue( g_pjcomm, 'C', "use_cont_num",             stClear.USE_CONT_NUM                );
		JSON_SetValue( g_pjcomm, 'C', "nonpay_active_flag",       m_stClearCancel.NONPAY_ACTIVE_FLAG  );
		JSON_SetValue( g_pjcomm, 'C', "active_rslt_desc",         m_stClearCancel.ACTIVE_RSLT_DESC    );
		JSON_SetValue( g_pjcomm, 'C', "nonpay_active_dept",       stClear.NONPAY_ACTIVE_DEPT          );
		JSON_SetValue( g_pjcomm, 'C', "nonpay_manage_zone",       stClear.NONPAY_MANAGE_ZONE          );
		JSON_SetValue( g_pjcomm, 'C', "nonpay_center_cd",         m_stClearCancel.NONPAY_CENTER_CD    );
		JSON_SetValue( g_pjcomm, 'C', "taker_nm",                 m_stClearCancel.TAKER_NM            );
		JSON_SetValue( g_pjcomm, 'C', "cut_job_flag",             m_stClearCancel.CUT_JOB_FLAG        );
		JSON_SetValue( g_pjcomm, 'C', "mtr_num",                  m_stClearCancel.MTR_NUM             );
		...
		callBackfunc = TR81208; //통신완료 후에 호출 함수
		HTTP_DownloadData( szUrl, "FR81208_IN",  "FR81208" , sndbuf, szbuf );
...
//------------------------------------------------------------------
	long TR81208(void)
	{
		...
		if(g_Chk_Json(81208) >= 0)
		{
			Mem_Set((byte*)szSql, 0x00, sizeof(szSql));	
			SPRINT(szSql, "DELETE FROM C6202_RELEASE WHERE MTR_NUM = '%s'", m_stClearCancel.MTR_NUM, 0, 0);
