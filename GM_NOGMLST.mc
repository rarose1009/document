...
//검색
	...
	#define		GRID_H_P	(CHT+52)			// table 행 높이 PDA
	...
	DlgObject	DlgRes[] = 
	{
		...
		//타이틀
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "순수 미검침 리스트"),
	...
	DlgObject	DlgRes_P[] = 
	{
		...
		//타이틀
		DLG_TEXT(P_TX, P_TY, P_STWD, P_STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, WHITE, TRANSPARENT, TXT_TITLE, "순수 미검침 리스트"),
...
//PRINT(">>>>>> lParam : %d , i : %d",lParam,i,0);
		if( i > -1 )
		{
			g_Sql_SaveSeq( "SELECT ROWID FROM GUMDATA WHERE SEND_YN = 'N' OR SEND_YN = '' OR SEND_YN IS NULL ORDER BY DONG_ORD, HOUSE_NUM_ORD, MTR_COURSE", SREACH_FILE);
...
//------------------------------------------------------------------------------------------
	bool REDRAW(void)
	{
		...
		g_Sql_SaveSeq( "SELECT ROWID FROM GUMDATA WHERE SEND_YN = 'N' OR SEND_YN = 'I' OR SEND_YN = '' OR SEND_YN IS NULL ORDER BY DONG_ORD, HOUSE_NUM_ORD, MTR_COURSE", SREACH_FILE);
...
//------------------------------------------------------------------------------------------
	...
	from gumdata WHERE SEND_YN = 'N' OR SEND_YN = 'I' OR SEND_YN = '' OR SEND_YN IS NULL ORDER BY HOUSE_NUM_ORD, DONG_ORD, MTR_COURSE"
								);
			}
			else
			{
	...
	from gumdata WHERE SEND_YN = 'N' OR SEND_YN = 'I' OR SEND_YN = '' OR SEND_YN IS NULL ORDER BY HOUSE_NUM_ORD, DONG_ORD, MTR_COURSE"
			...
			JSON_SetValue	(h		, 'C', "title"			, "순수 미검침 리스트");
