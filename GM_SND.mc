...
//	#define USE_ON_SELECT
	...
	DlgObject	DlgRes[] = 
	{
		...
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, EDITSTY_BORDER, EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "정기 검침 자료 송신"),
		
		DLG_TEXT(  STARTX,		STARTY+ 65, 1000,  70, 0, 0, 0, EDITSTY_BORDER, RED, TXTINCTRLBK, TXT_DATA13, "정기 검침완료 자료를 송신하세요."),
	...
	void REDRAW(void)
	{
		...
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE <> '2' AND MTR_WORK_CODE <> 'L' AND (SEND_YN = 'Y' OR SEND_YN = 'S')", 0 ,0 ,0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		...
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE SEND_YN = 'Y' AND MTR_WORK_CODE <> 'L' ", 0, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		EditCtrl_SetStr( Get_hDlgCtrlByID(TXT_DATA4), sztmp );
		
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE <> '2' AND MTR_WORK_CODE <> 'L' AND SEND_YN = 'Y'", 0, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		...
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE SEND_YN = 'S' AND MTR_WORK_CODE <> 'L'", 0, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		...
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE <> '2' and MTR_WORK_CODE <> '' AND MTR_WORK_CODE <> 'L' AND SEND_YN = 'S'", 0, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		EditCtrl_SetStr( Get_hDlgCtrlByID(TXT_DATA8), sztmp );
		
		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE = '2' AND MTR_WORK_CODE <> 'L' AND SEND_YN = 'S'", 0, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		...
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		Str_Cat(szToday, "%");
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE INDI_DATE LIKE '%s' AND MTR_WORK_CODE <> 'L'",szToday, 0, 0   );
		g_Sql_RetStr(szSql, 10, sztmp );
		EditCtrl_SetStr( Get_hDlgCtrlByID(TXT_DATA10), sztmp );

		Mem_Set( (byte*)szSql, 0x00, sizeof(szSql) );
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		SPRINT( szSql ,"SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE <> '2' AND MTR_WORK_CODE <> 'L' AND INDI_DATE LIKE '%s' AND (SEND_YN = 'Y' OR SEND_YN = 'S')",szToday, 0, 0 );
		g_Sql_RetStr(szSql, 10, sztmp );
		...
		/*********************/
		Mem_Set( (byte*)sztmp, 0x00, sizeof(sztmp) );
		g_Sql_RetStr("SELECT COUNT(*) FROM GUMDATA WHERE MTR_WORK_CODE = 'L'", 10, sztmp );
	...
	void GM_DATASAND(void)
	{
		...
		if( Str_Cmp(szzone_manage_num, "" ) == 0 )
		{
			Mem_Set( (byte*) szSql, 0x00, sizeof( szSql) );
		...
		SPRINT( szSql, "SELECT PARAM4 FROM RCV_LOG WHERE GUBUN = '1' ", 0, 0, 0 );
		g_Sql_RetStr(szSql, 6, szbranch_cd);
				
		Mem_Set( (byte*) szSql, 0x00, sizeof( szSql) );
		Mem_Set( (byte*) szcenter_cd, 0x00, sizeof( szcenter_cd) );
		SPRINT( szSql, "SELECT PARAM5 FROM RCV_LOG WHERE GUBUN = '1' ", 0, 0, 0 );
		g_Sql_RetStr(szSql, 2, szcenter_cd);
		
		Mem_Set( (byte*) szSql, 0x00, sizeof( szSql) );
		Mem_Set( (byte*) szgmtr_meths, 0x00, sizeof( szgmtr_meths) );
		SPRINT( szSql, "SELECT PARAM7 FROM RCV_LOG WHERE GUBUN = '1' ", 0, 0, 0 );
	...
	void Send_Photo(void)
	{
		...
		if(nRet > 0)
		{
			...
			SPRINT( szSql, "SELECT INSP_MEMO FROM GUMDATA WHERE MTR_NUM = '%s' ", m_szMtrnum, 0, 0 );
		...
		else
		{
			...
			g_Sql_RetInt("SELECT COUNT(*) FROM GUMDATA WHERE INSP_SAVE_YN = 'S' AND INSP_YN = 'Y'" , &lCnt );
			...
			if( lCnt > 0 )
			{
				...
				Mem_Set( (byte*)szTmp, 0x00, sizeof(szTmp) );
				g_Sql_RetStr("SELECT MTR_NUM FROM GUMDATA WHERE INSP_SAVE_YN = 'S' AND INSP_YN = 'Y' LIMIT 0,1", 20, szTmp );
			...
			else
			{
				...
				g_Sql_DirectExecute( "UPDATE GUMDATA SET SEND_YN = 'Y' WHERE SEND_YN = 'S' AND MTR_WORK_CODE <> 'L'" );
	...
	long TR24263(void)
	{
		...
		if(g_Chk_Json( 24263 ) >= 0)
		{
			...
			{
				Mem_Set((byte*)szDirPath, 0x00, sizeof(szDirPath));
				SPRINT(szDirPath, "UPDATE GUMDATA SET INSP_SAVE_YN = 'Y' WHERE MTR_NUM = '%s' AND INSP_SAVE_YN = 'S' AND INSP_YN = 'Y' ", m_szMtrnum, 0, 0 );
			...
			{
				Mem_Set((byte*)szDirPath, 0x00, sizeof(szDirPath));
				SPRINT(szDirPath, "UPDATE GUMDATA SET INSP_SAVE_YN = 'Y' WHERE MTR_NUM = '%s' AND INSP_SAVE_YN = 'S' AND INSP_YN = 'Y' ", m_stInspdata.szMtr_num, 0, 0 );
				g_Sql_DirectExecute( szDirPath );
			}
			...
			if(lRet <= 0)
			{
				...
				lRet = 0;
				g_Sql_RetInt("SELECT COUNT(*) FROM GUMDATA WHERE INSP_SAVE_YN = 'S' AND INSP_YN = 'Y'" , &lRet );
			}
	...
	void Set_InspData(char* szMtrnum)
	{
		...
		hstmt = sql->CreateStatement(sql, "SELECT MTR_NUM, INDI_INSP_VM, INDI_INSP_VA, INDI_INSP_VC, INSP_YMD \
												, INSP_MEMO \
										     FROM GUMDATA WHERE MTR_NUM = ? AND INSP_SAVE_YN = 'S' AND INSP_YN = 'Y'"
		);
...
Finally:
	...
	long TR26001( void )
	{
		...
		if( g_Chk_Json(26001) >= 0 )
		{
			...
			// 1. 데이터 업데이트
			Mem_Set( (byte*)szTmp, 0x00, sizeof(szTmp) );
			SPRINT( szTmp, "UPDATE GUMDATA SET PHOTO_MTR_YN = 'Y' WHERE REQ_YM = '%s' AND DEADLINE_FLAG = '%s' AND MTR_NUM = '%s'", m_szReqym, m_szDeadlineflag, m_szMtrnum );
	...
	long TR26002( void )
	{
		...
		if( g_Chk_Json(26002) >= 0 )
		{
			...
			{	
				SPRINT( szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VC_YN = 'Y' WHERE REQ_YM = '%s' AND DEADLINE_FLAG = '%s' AND MTR_NUM = '%s'", m_szReqym, m_szDeadlineflag, m_szMtrnum );
				g_Sql_DirectExecute( szTmp ); 
			}
			else if( Str_Cmp(m_szCompenstype, "2" ) == 0  )
			{
				SPRINT( szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VA_YN = 'Y' WHERE REQ_YM = '%s' AND DEADLINE_FLAG = '%s' AND MTR_NUM = '%s'", m_szReqym, m_szDeadlineflag, m_szMtrnum );
				g_Sql_DirectExecute( szTmp ); 
			}
			else if( Str_Cmp(m_szCompenstype, "3" ) == 0  )
			{
				SPRINT( szTmp, "UPDATE GUMDATA SET PHOTO_COMPENS_VM_YN = 'Y' WHERE REQ_YM = '%s' AND DEADLINE_FLAG = '%s' AND MTR_NUM = '%s'", m_szReqym, m_szDeadlineflag, m_szMtrnum );
				g_Sql_DirectExecute( szTmp ); 
			}
