...
//	#define USE_ON_TIMER
	...
	DlgObject	DlgRes[] = 
	{
		...
		//타이틀
		DLG_TEXT(TX, TY, STWD, STHT, 0, 0, 0, EDITSTY_BORDER|EDITSTY_BOLD, TTLCTRLFR, TTLCTRLBK, TXT_TITLE, "리스트 테스트"),
	...
	{	
		...
		{					
			case BID_QUICK:
			{
				//퀵뷰 호출
				//ListTest();
				//MessageBoxEx (CONFIRM_OK, "메세지박스 테스트...");
...
//=======================================================================================
	...
	{		
		...
		m_hGrid = GridCtrl_CreateObject(DMS_X(20), DMS_Y(170), DMS_X(500), DMS_Y(170+800), 200, 5);
		
		GridCtrl_SetScrollDelta(m_hGrid, DMS_Y(800));
		FrmCtrl_SetVarScrollThickness(m_hGrid, DMS_X(60));
		
		
		GridCtrl_SetColumn(m_hGrid, 0, "필드1", DMS_X(190), 0, GCTYPE_CHKCTRL	, 32);
		GridCtrl_SetColumn(m_hGrid, 1, "필드2", DMS_X(190), 0, GCTYPE_TEXTCTRL	, 32);
		GridCtrl_SetColumn(m_hGrid, 2, "필드3", DMS_X(170), 0, GCTYPE_TEXTCTRL	, 32);
		GridCtrl_SetColumn(m_hGrid, 3, "필드4", DMS_X(150), 0, GCTYPE_BUTCTRL	, 32);
		GridCtrl_SetColumn(m_hGrid, 4, "필드5", DMS_X(270), 0, GCTYPE_TEXTCTRL	, 32);
		
		//long GridCtrl_SetColumnStyle(stGridCtrl* pGrd, int index, int add_style, int rem_style, int font, int fr_color, int bk_color)
		//GridCtrl_SetColumnStyle(h, 0, 0, 0, 30, 0, 0);
		
		GridCtrl_SetHeaderHeight(m_hGrid, DMS_Y(70));
		GridCtrl_SetBodyHeight(m_hGrid, DMS_Y(60));
